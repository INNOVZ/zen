...
export const useDashboardDataOptimized = (userId?: string) => {
  const {
    connectionStatus,
    setLoading,
    orgChatbots,
  ...
  return {
    // Data from store (for backward compatibility)
    ...
    stats: {
      ...
      totalConversations: data.conversationCount || conversations?.length || 0,
