(function() {
  ...
  // Configuration
  const config = {
    // Backend API URL - read from script attribute or use default
  ...
  function convertLegacyUrl(url) {
    ...
    if (url.includes('storage/v1/object/uploads/')) {
      ...
      if (pathParts.length > 1) {
        ...
        const baseUrl = config.apiUrl; // Use backend URL for proxy
  ...
  async function loadChatbotConfig() {
    try {
      // Use the public API endpoint to get chatbot config
      if (config.chatbotId) {
  ...
  window.zentriaSendMessage = function() {
    ...
    // Send to API using public endpoint (no authentication required)
